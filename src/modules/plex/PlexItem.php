...
class PlexItem {
     ...
     * @return array|null
     */
    public static function getMovie($rPlexID, $rTMDBID) {
     ...
     * @return array|null
     */
    public static function getEpisode($rPlexID, $rTMDBID, $rSeason, $rEpisode) {
    ...
    /**
     * Сохранить категорию во временный файл.
     *
     * @param string $rType
    ...
    /**
     * Извлечь теги из XML-данных Plex.
     *
     * @param array $data
     * @param string $tag
     * @param int $limit
     * @return string
     */
    public static function extractTags(array $data, string $tag, int $limit): string {
        $items = [];
        foreach (self::makeArray($data[$tag] ?? []) as $item) {
     ...
     */
    public static function getTmdbIdFromPlex($rContent) {
        $tmdbId = null;
        $language = null;

        // 1. Try a new format: separate <Guid id="tmdb://12345"/>
        if (isset($rContent['Guid']) && is_array($rContent['Guid'])) {
            foreach (self::makeArray($rContent['Guid']) as $guid) {
    ...
    /**
     * Основная точка входа обработки элемента Plex.
     * Заменяет loadcli().
     */
    public static function run() {
        ...
        if ($rImportArray['type']) {
            ...
            if ($rThreadData['type'] == 'movie') {
                ...
                $rContent = json_decode(json_encode(simplexml_load_string(self::readURL($rURL))), true);
                ...
                foreach (self::makeArray($Video['Media']) as $rMedia) {
                    foreach (self::makeArray($rMedia['Part']) as $rPart) {
                ...
                if (!empty($rFileArray['file'])) {
                    ...
                    if (in_array($rInternalPath, $rStreamDatabase) || in_array($rDirectURL, $rStreamDatabase)) {
                        ...
                        foreach (array_slice(self::makeArray($Video['Director']), 0, 3) as $rMember) {
                            $rDirectors[] = $rMember['@attributes']['tag'];
                        }
                        $rGenres = array();
                        foreach (array_slice(self::makeArray($Video['Genre']), 0, $rThreadData['max_genres']) as $rGenre) {
                        ...
                        if (empty($rCategoryIDs)) {
                            ...
                            foreach ($rGenres as $rGenreTag) {
                                if (isset($rPlexCategories[3][$rGenreTag])) {
                                    ...
                                    self::addCategory($rThreadData['type'], $rGenreTag);
                                }
                            }
                        }
            ...
            if ($rThreadData['type'] == 'show') {
                ...
                $rSeasons = self::makeArray(json_decode(json_encode(simplexml_load_string(self::readURL($rURL))), true)['Directory']);
                ...
                $rEpisodes = self::makeArray(json_decode(json_encode(simplexml_load_string(self::readURL($rURL))), true)['Video']);
                ...
                $rSeries = self::getSeriesByID($rThreadData['uuid'], $rTMDBID);
                ...
                if (!$rSeries) {
                    ...
                    $rSeriesArray['cast'] = self::extractTags($rShowData, 'Role', 5);
                    $rSeriesArray['director'] = self::extractTags($rShowData, 'Director', 3);
                    $rSeriesArray['genre'] = self::extractTags($rShowData, 'Genre', 3);

                    if (count($rCategoryIDs) == 0) {
                        if (0 < $rThreadData['max_genres']) {
                            $rParsed = array_slice(self::makeArray($rShowData['Genre']), 0, $rThreadData['max_genres']);
                        } else {
                            $rParsed = self::makeArray($rShowData['Genre']);
                        }
                ...
                foreach ($rEpisodes as $rEpisode) {
                    if ($rEpisode['@attributes']['parentIndex'] && $rEpisode['@attributes']['index']) {
                        ...
                        $rFirstFile = null;
                        $rFileArray = array('file' => null, 'size' => null, 'data' => null, 'key' => null);
                        foreach (self::makeArray($rEpisode['Media']) as $rMedia) {
                        ...
                        if (!empty($rFileArray['file'])) {
                            ...
                            $rUpgradeData = self::getEpisode($rThreadData['uuid'], ($rThreadData['check_tmdb'] ? $rTMDBID : null), $rReleaseSeason, $rReleaseEpisode);
                            if (!$rUpgradeData) {
